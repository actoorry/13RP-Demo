...
- **决策演示场景（保留）**：核心供应商「包头北方稀土矿业」突发停产 N 天 → 342 条路径推演 → 3 方案（P1 切换备用供应商 / P2 启用战略库存+调整生产节拍 / P3 替代材料方案）→ 4 博弈因素（备用产能/同行抢购/恢复不确定/客户接受替代）→ 5 条指令（合同要素字段）；演示接口 `/api/demo/**` 与 `/ws/**` **匿名放行**（演示场景无需登录），管理端 `/api/**` 需 JWT
- **权威文档**（在 E:\13RP 仓库）：
  - `02_产品需求文档PRD/功能需求_P0-P3.md`（九大业务域 F-0.1~F-0.9）
