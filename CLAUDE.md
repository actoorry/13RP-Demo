...
- **管理端导航（V0.4 五模块归位）**：共用底座（基础数据与系统管理/流程引擎/待办事宜）+ 采购模块（采购域）+ 销售模块（销售域+CRM）+ 库存模块（库存域）+ 财务模块（财务域）+ 人力模块（组织与权限）；路由路径沿用 base/org/purchase/sale/inventory/finance/crm/flow/todo
- **权威文档（在 E:\13RP 仓库）**：
